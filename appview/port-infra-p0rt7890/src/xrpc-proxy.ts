...
//
// Original path: appview/port-infra-p0rt7890/svelte/src/routes/xrpc/[...path]/+server.ts
//
// This was a SvelteKit route handler (imports `json`/`RequestEvent` from
// '@sveltejs/kit' and `RequestHandler` from its generated './$types'). It will
// NOT run as-is now that the SvelteKit build (and the '@sveltejs/kit'
// dependency that supplied those types) is gone from this repository.
//
// docs/operator-quickstart.md section 2 (measured 2026-08-15, before this
// migration) found that THIS file -- not appview/port-infra-p0rt7890/src/app.ts
// -- was the one actually reached by the Cloudflare Worker deploy (via the
// SvelteKit-built worker that wrangler.jsonc's old "main" field pointed at),
// proxying XRPC calls to AGENTGATEWAY_MCP_ROUTER_URL (mcp.etzhayyim.com) as an
// MCP tools/call. src/app.ts proxies to a different upstream (DISPATCHER_URL,
// dispatcher.etzhayyim.com) and was, per that same measurement, not reached by
// any deployed route.
//
// Whether/how to revive this behavior (e.g. as a plain Cloudflare Worker
// fetch handler co-located with static-asset serving) is an undecided product
// question that this migration does not resolve. It is kept here, byte-
// identical to the original, purely so the logic is not lost.
